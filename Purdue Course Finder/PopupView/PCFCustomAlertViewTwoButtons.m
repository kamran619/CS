//
//  PCFCustomAlertViewTwoButtons.m
...
@implementation PCFCustomAlertViewTwoButtons
@synthesize buttonOne, buttonTwo,title,body,delegate;
...
#define buttonOneOriginY (self.frame.size.height - buttonOneSizeHeight - 10)
#define buttonOneSizeWidth (((self.frame.size.width - (2*buttonOneOriginX)))/2) - 10
#define buttonOneSizeHeight 40
#define buttonTwoOriginX (buttonOneOriginX + buttonOneSizeWidth + 20)
#define buttonTwoOriginY (self.frame.size.height - buttonTwoSizeHeight - 10)
#define buttonTwoSizeWidth buttonOneSizeWidth//(((self.frame.size.width - (2*buttonOneOriginX)))/2) - 15
...
-(id) initAlertView:(CGRect)frame:(NSString *)title:(NSString *)body:(NSString *)buttonOneTitle:(NSString *)buttonTwoTitle
{
    ...
    [self.buttonOne setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];//colorWithRed:0.0993145 green:0.0957361 blue:0.562879 alpha:1]
    ...
    [self.buttonTwo setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];//colorWithRed:0.0993145 green:0.0957361 blue:0.562879 alpha:1]
...
-(void)buttonPushed:(id)sender {
    if (delegate) {
        if ([delegate respondsToSelector:@selector(clickedYesOnTwoButton)]) [delegate clickedYesOnTwoButton];
    }
    [super dismiss];
}
...
{
    if (delegate) {
        if ([delegate respondsToSelector:@selector(clickedNoOnTwoButton)]) [delegate clickedNoOnTwoButton];
    }
    [super dismiss];
